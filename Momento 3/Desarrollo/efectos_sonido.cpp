...
efectos_sonido::efectos_sonido(QObject *parent) {
  ...
  sonido_explosion->setSource(QUrl("qrc:/new/efectos_sonido/Material/"
                                   "Efectos_sonido/bomba_efecto_sonido.wav"));
  sonido_explosion->setVolume(1.0);

  sonido_herido = new QSoundEffect(parent);
  sonido_herido->setSource(QUrl("qrc:/new/efectos_sonido/Material/"
                                "Efectos_sonido/herido_efecto_sonido.wav"));
  ...
  musica_nivel->setSource(
      QUrl("qrc:/new/efectos_sonido/Material/Efectos_sonido/"
           "musica_scena_efecto_sonido.wav"));
  salida_audio->setVolume(0.5);
  musica_nivel->setLoops(
      QMediaPlayer::Infinite); // Se repite siempre que se termine
}
